<template>
  <div class="overflow-x-auto w-full">
    <table class="table">
      ...
      <tbody>
        <tr  v-for="(project, index) in projectsStore.projectWithCompletion" :key="project.id" class="hover">
          <th>{{index +1}}</th>
          <td>{{project.name}}</td>
          <td>{{ project.completed }} / {{project.taskCount}}</td>
          <td><progress class="progress progress-primary w-56" :value='project.completion' max="100"></progress></td>
        </tr>
      </tbody>
    </table>
  </div>
